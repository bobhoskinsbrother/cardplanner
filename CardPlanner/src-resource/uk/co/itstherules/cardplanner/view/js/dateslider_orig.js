...
DateSlider = Class.create({
	...
	initialize : function(p_sBarId, p_sStartDate, p_sEndDate, p_iStartYear, p_iEndYear, options) {
		...
		this.barId = p_sBarId;
		this.shiftPanel = p_sBarId + 'ShiftPanel';
		this.lefthandle = p_sBarId + 'LeftHandle';
		this.righthandle = p_sBarId + 'RightHandle';
		
		this.shiftPanelClass = 'shiftPanel';
		this.leftHandleClass = 'leftHandle';
		this.rightHandleClass = 'rightHandle';
	...
	createSliderBar : function(p_sBarId) {
		 ...
		 * - Loop through years/months to change the grid
	...
	_bgStopDrag : function() {
		/* Move? */
		l_iDiff = $(sliderReference.righthandle).offsetLeft + ($(sliderReference.barId).offsetLeft-600);
		
		if(l_iDiff > -2) {
		/* Move the bgbar */
		var l_iLeft = '-'+($(sliderReference.righthandle).offsetLeft-590)+'px';
		new Effect.Morph(sliderReference.barId, { style: {left: l_iLeft}, duration:.5});
		}	
	...
	createHandles : function(p_sBarId, p_sStartDate, p_sEndDate) {
		...
		l_oLeftHandle = new Element('span', {className: sliderReference.leftHandleClass, id : sliderReference.lefthandle, style:'left:'+this.iLeftOffsetLH+'px'}).update('&nbsp;');		
		l_oRightHandle = new Element('span', {className: sliderReference.rightHandleClass, id : sliderReference.righthandle, style:'left:'+this.iLeftOffsetRH+'px'}).update('&nbsp;');
	...
	dragShiftPanel : function() { 
  		/* Set the handlers while dragging the sliderReference.shiftPanel */
  		$(sliderReference.lefthandle).setStyle({left: ($(sliderReference.shiftPanel).offsetLeft-sliderReference.sliderBarMargin)+'px'});
  		$(sliderReference.righthandle).setStyle({left: ($(sliderReference.shiftPanel).offsetLeft + $(sliderReference.shiftPanel).offsetWidth-sliderReference.sliderBarMargin)+'px'});						
	...
	createShiftPanel : function(p_sBarId, p_sStartDate, p_sEndDate) {
		...
		l_oShiftPanel = new Element('div', {id : sliderReference.shiftPanel, className: sliderReference.shiftPanelClass, style:'left:'+(this.iLeftOffsetLH)+'px; width:'+l_iBarWidth+'px'});
	...
	handleLimitPos: function(x, y, drag) 
		{
		 inbox = drag.element.getDimensions();
		 outbox = Element.getDimensions(drag.element.parentNode);
		 maxPos = drag.element.hasClassName(sliderReference.leftHandleClass) ?
			parseInt($(sliderReference.righthandle).style.left)-inbox.width : outbox.width - inbox.width;
		 
		 minPos = drag.element.hasClassName(sliderReference.rightHandleClass) ?
		 parseInt($(sliderReference.lefthandle).style.left)+inbox.width : 0;
		 return [ x > maxPos ? maxPos : (x < minPos ? minPos : x), y];
	},
	_setDates : function() {
		/* Get the position of the handles */
		l_iLeftPos = $(sliderReference.lefthandle).offsetLeft/this.options.dayWidth;
		l_iRightPos = $(sliderReference.righthandle).offsetLeft/this.options.dayWidth;
	...
	_rightDrag : function (e, ev) {
		...
		l_panelLength = $(sliderReference.righthandle).offsetLeft - $(sliderReference.lefthandle).offsetLeft - 5;
		$(sliderReference.shiftPanel).setStyle({width : (l_panelLength+2*sliderReference.sliderBarMargin)+'px'});																	
		sliderReference._setDates();	
	},
	_leftDrag : function(e, ev) {
		if(ev.type == "mouseup" && sliderReference.options.onEnd != null) sliderReference.options.onEnd();
		l_panelLength = $(sliderReference.righthandle).offsetLeft - $(sliderReference.lefthandle).offsetLeft - 4;
		$(sliderReference.shiftPanel).setStyle({left: ($(sliderReference.lefthandle).offsetLeft+4)+'px', width : l_panelLength+'px'});															
		sliderReference._setDates();		
	},
	morphTo : function (p_oDateStart, p_oDateEnd) {
		...
		$(sliderReference.lefthandle).morph('left:'+l_offsetLeftLH+'px');
		$(sliderReference.righthandle).morph('left:'+l_offsetLeftRH+'px');
		$(sliderReference.shiftPanel).morph('width : '+(l_panelLength+2*sliderReference.sliderBarMargin)+'px; left : '+(l_offsetLeftLH+2)+'px');																			
	},
	...
	centerBar : function() {
		...
		var l_iShiftContainerWidth = $(sliderReference.barId).up().getWidth();
		
		$(sliderReference.barId).setStyle({left:(this.iLeftOffsetLH-(2*this.iLeftOffsetLH)+(l_iShiftContainerWidth/2)-(l_iPanelWidth/2))+'px'});
	}
});