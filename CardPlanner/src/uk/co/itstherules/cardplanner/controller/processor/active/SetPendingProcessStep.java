...
public final class SetPendingProcessStep extends BaseProcessStep<Entity<?>> {

	public static final String KEY = "SetPending";
	...
	public Entity<?> process(ObjectCache objectCache, ValuesProvider provider, HttpServletResponse response, ModelViewRegister viewFactory, QueryKeyViolations violations) {
		Object reply = processFirstInboundStep(objectCache, provider, response, viewFactory, violations);
		Assertion.checkIsInstance(Entity.class, reply);
		Entity<?> entity = Entity.class.cast(reply);
	    entity.pending();
	    return entity;
    }
