...
public final class ProcessPageWithEntityProcessStep extends BaseProcessStep<String> {
	
	public static final String KEY = "ProcessPageWithEntity";
	...
	public String process(ObjectCache objectCache, ValuesProvider provider, HttpServletResponse response, ModelViewRegister viewFactory, QueryKeyViolations violations) {
		...
		for (ProcessStep<?> processStep : inputSteps) {
	        ...
	        Assertion.checkIsTrue(set, MessageFormat.format("Process {0} was expected to produce either a PageModel, or Entity.  It did neither.  It produced {1}", processStep.getKey(), reply));
        }
